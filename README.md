...
Call the `logout()` method in the action of your **Logout** button. Once the session cookie has been cleared, [delete the user's credentials](examples/credentials-manager/clear.md).
...
For a full guide including configuration steps and reading the raw claim value, see the [IPSIE session expiry examples](examples/credentials-manager/ipsie-session-expiry.md).
...
> [!NOTE]
> For complete MFA implementation examples including SMS, email, OTP, and push notifications, see the [**MFA examples**](examples/mfa-api/README.md).
